...
const API_BASE = "https://fyjob.my.id/api";
...
async function apiRequest(endpoint, method = "GET", body = null) {
  ...
  if (res.status === 401) {
    ...
    // If refresh fails, try re-syncing auth from the dashboard tab
    if (!refreshedToken) {
  ...
  // Read body as text first so we can handle both JSON and non-JSON (HTML) responses
  const rawText = await res.text();
  let data;
  try {
    data = JSON.parse(rawText);
  } catch {
    // Server returned HTML or empty body — likely a CDN/proxy intercept or wrong URL
    throw new Error(`Server tidak merespons JSON (status ${res.status}). Cek koneksi atau API URL.`);
  }
